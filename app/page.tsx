...
// ─── 科学探索屋：教科版小学科学三年级上册第三单元《物体的运动》8 课 ───
// 首页对话框下方展示为一排快捷按钮，学生点一下即以对应课时需求直接生成整堂课。
const SCIENCE_UNIT_TOPICS: { label: string; title: string }[] = [
  { label: '运动和位置', title: '运动和位置' },
  { label: '各种各样的运动', title: '各种各样的运动' },
  { label: '直线运动和曲线运动', title: '直线运动和曲线运动' },
  { label: '物体在斜面上运动', title: '物体在斜面上运动' },
  { label: '相同距离比快慢', title: '比较相同距离内运动的快慢' },
  { label: '相同时间比快慢', title: '比较相同时间内运动的快慢' },
  { label: '我们的“过山车”', title: '我们的“过山车”' },
  { label: '测试“过山车”', title: '测试“过山车”' },
];

const buildUnitRequirement = (courseTitle: string) =>
  `请为小学三年级学生生成科学课《物体的运动》中的一课：“${courseTitle}”，` +
...
function HomePage() {
  ...
  const handleGenerate = async (overrideRequirement?: string) => {
    ...
    if (preparingGenerate) return;
    // 主题快捷按钮直接把需求文本传进来；其余入口回退到输入框当前值。
    const requirementText = (overrideRequirement ?? form.requirement).trim();
    if (!requirementText) {
  ...
  // 点击「单元主题」快捷按钮：把主题需求写进输入框（保留可见草稿），并立即触发生成。
  const handleTopicSelect = (unitRequirement: string) => {
    if (preparingGenerate) return;
    if (!hasUsableProvider) {
      // 未配置模型时不去生成，直接打开设置面板引导配置。
      setSettingsOpen(true);
      return;
    }
    updateRequirementCache(unitRequirement);
    setForm((prev) => ({ ...prev, requirement: unitRequirement }));
    void handleGenerate(unitRequirement);
  };
      ...
      >
        ...
        <div className="relative" data-pro-morph="lockup">
          <motion.h1
            ...
            className="mb-2 bg-gradient-to-r from-[#12E3BE] via-[#22C7F5] to-[#3B9EFF] bg-clip-text text-4xl font-black leading-none tracking-tight text-transparent md:text-6xl"
          >
            科学探索屋
          </motion.h1>
        ...
        >
          ...
          >
            ...
            <div className="flex flex-wrap gap-1.5 px-3 pb-2">
              {SCIENCE_UNIT_TOPICS.map((topic) => (
                <Suggestion
                  key={topic.label}
                  suggestion={topic.label}
                  onClick={() => handleTopicSelect(buildUnitRequirement(topic.title))}
